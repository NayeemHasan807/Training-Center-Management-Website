<?php
	...
	if (isset($_GET['id'])) {
		$mail = getmailbyid($_GET['id']);	
	}else{
		header('location: trainerViewMail.php');
	}
...
<html>
<head>
	<title>Trainer Edit Mail</title>
	<script type="text/javascript" src="../assets/js/trainerEditMail.js"></script>
</head>
<body>
	<fieldset>
		...
		<table cellspacing="0" cellpadding="5" border="1" width="100%">
			<tr>
				...
				<td>
					<form id="form">
						Subject:<br/>
						<input type="text" id="mailsubject" name="mailsubject" value="<?=$mail['mailsubject']?>" onclick ="click1()" onkeyup="validatesubject()"><br/>
						<div id="show1">
						</div>
						To:<br/>
						<select id="towhom" name="towhom" onclick="click2()" value="<?=$mail['towhom']?>">
						...
						Body:<br/>
						<textarea rows="5" cols="80" id="mailbody" name="mailbody" onclick ="click3()" onkeyup="validatebody()">
							<?=trim($mail['mailbody']);?>
						</textarea><br/>
						<div id="show3">
						</div>
						<a href="trainerViewMail.php">
							<button type="button">
								Back
							</button>
						</a>
						<input type="hidden" id="id" name="id" value="<?=$mail['id']?>">
						<input type="button" id="updatemail" name="updatemail" value="Update" onclick="click1(),click2(),click3(),validate()">
						<div id="output">
						</div>
					</form>
				</td>

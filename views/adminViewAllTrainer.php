		    ...
		    <td colspan="10">
				...
				<td align="Center">
					<table border="1" width="100%" cellspacing="0" cellpadding="5">
						...
						?>
						<?php
							$trainers = getalltrainer();
							for ($i=0; $i != count($trainers); $i++)
							{
						?>
						<tr>
							<td><?=$trainers[$i]['trainerid']?></td>
							<td><?=$trainers[$i]['name']?></td>
							<td><?=$trainers[$i]['email']?></td>
							<td><?=$trainers[$i]['gender']?></td>
							<td><?=$trainers[$i]['dob']?></td>
							<td><?=$trainers[$i]['phoneno']?></td>
							<td><?=$trainers[$i]['address']?></td>
							<td><?=$trainers[$i]['eduqualification']?></td>
							<td>
								<a href="adminEditTrainer.php?trainerid=<?=$trainers[$i]['trainerid']?>">Edit</a>
								<a href="adminDeleteTrainer.php?trainerid=<?=$trainers[$i]['trainerid']?>">Delete</a> 
							</td>
						</tr>
